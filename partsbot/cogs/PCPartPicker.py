...
def get_price(url):
    ...
    if not realtext == 'Verification':
        ...
        return prices, links, sellers, images
    else:
        return 'rate_limited', 'rate_limited', 'rate_limited', 'rate_limited'
                ...
                if not productnames == 'rate_limited':
                    if not productnames == 'use_current':
                        if not len(productnames) == 0:
                            ...
                            if not item == 10:
                                with concurrent.futures.ThreadPoolExecutor() as pool:
                                    prices, links, sellers, images = await asyncio.get_event_loop().run_in_executor(pool,
                                                                                                            get_price,
                                                                                                            f"https://{country}pcpartpicker.com{producturls[item]}")
                                if not str(prices) == 'rate_limited':
                                    if len(prices) > 0:
                                        description = f"**[{sellers[0]}]({links[0]}): {prices[0]}**"
                                        for i in range(1, len(prices)):
                                            description = f"{description}\n[{sellers[i]}]({links[i]}): {prices[i]}"
                        ...
                        with concurrent.futures.ThreadPoolExecutor() as pool:
                            prices, links, sellers, images = await asyncio.get_event_loop().run_in_executor(pool, get_price,
